...
// 즉시 실행 async 함수(IIFE) 안에서 모든 초기화
(async () => {
  await checkLogin();
})();
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    document.title = post.title ? `${post.title}` : '게시글';
    ...
    try {
      ...
      if (comments.length === 0) {
        ...
        comments.forEach(comment => {
          ...
          if (comment.usid) {
            const loginIcon = document.createElement('img'); // 먼저 선언
            loginIcon.src = '/img/login-user.png';
            loginIcon.alt = '로그인 사용자';
            loginIcon.style.width = '14px';
            loginIcon.style.height = '14px';
            loginIcon.style.marginLeft = '4px';
            loginIcon.style.verticalAlign = 'middle';
            authorSpan.appendChild(loginIcon); // 나중에 붙이기
          }
          ...
          let displayTime = new Date(comment.time).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' });
          ...
          if (comment.replies && comment.replies.length > 0) {
            ...
            comment.replies.forEach(reply => {
              ...
              if (reply.uid) {
                ...
                loginIcon.style.marginLeft = '4px';
                loginIcon.style.verticalAlign = 'middle';
                replyAuthor.appendChild(loginIcon);
              }
              ...
              let displayTime = new Date(reply.time).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' });
              replyTime.className = 'time';
              replyTime.textContent = ` | ${displayTime}`;
    ...
    if (e.target.classList.contains('replyBtn')) {
      ...
      if (!replyBox) {
        ...
        replyBox.innerHTML = `
          ...
          <br/>
          <input type="text" class="replyAuthor" placeholder="이름" style="margin-top: 5px; display: ${loggedIn ? 'none' : 'inline-block'};" />
          <input type="password" class="replyPassword" placeholder="비밀번호" style="margin-top: 5px; margin-left: 5px; display: ${loggedIn ? 'none' : 'inline-block'};" />
          <br/>
