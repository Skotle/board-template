...
const selectedCategoryFromUrl = params.get('category') || 'all'; // 기본값 'all'
...
let categoryNames = []; // 서버에서 받아오는 번호별 카테고리 이름 배열

// 카테고리 번호 -> 이름 반환 함수
function getCategoryLabel(code) {
  const idx = Number(code);
  if (!isNaN(idx) && categoryNames.length > idx && categoryNames[idx]) {
...
function renderPosts(filteredPosts, isAdmin) {
  postList.innerHTML = '';
  ...
  filteredPosts.slice().reverse().forEach(post => {
    ...
    a.href = `/post.html?board=${boardName}&id=${post.id}&page=${}84`;

    // 카테고리 span
    if (post.category !== undefined) {
      const categorySpan = document.createElement('span');
      categorySpan.className = 'category';
      categorySpan.textContent = `[${getCategoryLabel(post.category)}]`;
    ...
    if (post.usid) {
      ...
      loginIcon.title = `${post.usid} 프로필로 이동`;
      ...
      loginIcon.setAttribute("onclick", `window.location.href='/profile.html?id=${post.usid}'`);
...
async function loadAndRenderPosts() {
  ...
  try {
    ...
    categoryNames = boardData.post_category || [];

    // 탭 활성화: URL 카테고리와 일치하는 버튼 활성화
    categoryTabs.forEach(btn => btn.classList.remove('active'));
    const activeTab = Array.from(categoryTabs).find(btn => btn.getAttribute('data-category') === selectedCategoryFromUrl);
    if (activeTab) activeTab.classList.add('active');
    else {
      const defaultTab = document.querySelector('#categoryTabs button[data-category="all"]');
      if (defaultTab) defaultTab.classList.add('active');
    }

    // 선택된 카테고리에 맞게 필터링 후 렌더링 (번호 문자열 기준 필터)
    const filtered = (selectedCategoryFromUrl === 'all')
      ? allPosts
      : allPosts.filter(post => String(post.category) === selectedCategoryFromUrl);

    renderPosts(filtered, window.isAdminFlag);
  } catch (err) {
...
categoryTabs.forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    const filtered = selectedCategory === 'all'
      ? allPosts
      : allPosts.filter(post => String(post.category) === selectedCategory);
